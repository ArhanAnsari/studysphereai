...
export default function SignInPage() {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      showError('OAuth sign in failed', error.message || 'Please try again.');
      setIsLoading(false);
    }
  };
  ...
  const handleAnonymousSignIn = async () => {
    ...
    try {
      ...
      success('Welcome Guest!', 'You can explore the app with limited features.');
      router.push('/dashboard');
    } catch (error: any) {
      showError('Anonymous sign in failed', error.message || 'Please try again.');
      setIsLoading(false);
    }
  };
      ...
      >
        ...
        <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 md:p-10">
          ...
          >
            ...
            <p className="text-blue-100/80">Sign in to continue your learning journey</p>
          </motion.div>
          ...
          >
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 px-6 py-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 rounded-xl font-medium text-white transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed group"
            >
          ...
          >
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 text-blue-100 hover:text-white border border-white/20 hover:border-white/40 rounded-xl transition-all duration-300 hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Continue as Guest
            </button>
          </motion.div>
